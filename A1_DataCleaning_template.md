...
``` r
names(demo)[1] <- "SUBJ" #take the 1st column and call it "SUBJ"
names(demo)[2] <- "VISIT"
```
...
``` r
LU$VISIT <- str_extract(LU$VISIT, "\\d") #in the LU-dataframe, under the column VISIT, only save the numbers
...
``` r
LU$SUBJ <- str_replace_all(LU$SUBJ, "[.]", "")  #in the column in g the dataframe, replace all "." with nothing
...
``` r
df1 <- merge(sub_demo, sub_LU) #merge the first 2
df <- merge(df1, sub_token) #add the last
```
...
``` r
visit1 <- filter(df, VISIT == 1) #use filter to only find the variables from visit 1

visit1_relevant <- select(visit1, SUBJ, ADOS, nonVerbalIQ, verbalIQ, Socialization) #choose only the relevant variables

names(visit1_relevant) <- c("SUBJ", "ADOS1", "nonVerbalIQ1", "verbalIQ1", "Socialization1") #rename the columns

df_merged <- merge(df, visit1_relevant) #merge with the dataframe with only the relevant variables
```
...
``` r
#rename participants
df_merged$SUBJ <- as.factor(df_merged$SUBJ) #make SUBJ a factor
levels(df_merged$SUBJ) #check the factors
```
...
``` r
levels(df_merged$SUBJ) <- c(1:61) # call the factors numbers from 1 to 61 instead of names


#rename Gender
df_merged$Gender <- as.factor(df_merged$Gender)
levels(df_merged$Gender)
```

    ## [1] "1" "2"

``` r
levels(df_merged$Gender) <- c("M", "F") #call first level "M" and second level "F"

#rename Diagnosis
df_merged$Diagnosis <- as.factor(df_merged$Diagnosis)
levels(df_merged$Diagnosis)
```

    ## [1] "A" "B"

``` r
levels(df_merged$Diagnosis) <- c("ASD", "TD")



#save data set as .csv file
write.csv(df_merged, file = "Assignment1_df.csv")
```
...
``` r
arrange1 <- df_merged %>% group_by(SUBJ) %>% filter(VISIT == 6) %>% arrange(desc(tokens_CHI)) #in new df with only visit 6 
#OR
arrange(df_merged, desc(VISIT), desc(tokens_CHI)) #arrange the existing dataframe
```

    ##     SUBJ VISIT Diagnosis        Ethnicity Gender   Age ADOS nonVerbalIQ
    ## 1     55     6        TD            White      M 41.93   NA          45
    ## 2     26     6       ASD            White      M 51.00   NA          46
    ## 3     18     6        TD            White      M 44.07   NA          50
    ## 4     25     6        TD            White      M 42.47   NA          48
    ## 5     43     6       ASD            White      M 57.37   NA          49
    ## 6     37     6       ASD            White      M 58.77   NA          50
    ## 7     13     6        TD            White      M 39.40   NA          47
    ## 8     22     6        TD            White      F 39.23   NA          43
    ## 9     14     6        TD            White      M 39.43   NA          44
    ## 10     4     6       ASD     White/Latino      M 51.37   NA          44
    ## 11    42     6        TD            White      M 40.13   NA          46
    ## 12    60     6       ASD            White      M 54.73   NA          50
    ## 13    16     6        TD            White      M 42.93   NA          49
    ## 14    41     6        TD            White      M 39.93   NA          45
    ## 15     2     6       ASD            White      M 49.70   NA          48
    ## 16     1     6        TD            White      M 40.13   NA          42
    ## 17    11     6        TD            White      M 40.27   NA          42
    ## 18    59     6        TD            White      M 41.00   NA          NA
    ## 19    49     6        TD            White      M 43.40   NA          45
    ## 20    12     6        TD            White      M 41.50   NA          44
    ## 21    58     6       ASD            White      M 46.07   NA          45
    ## 22    35     6        TD            White      M 39.07   NA          45
    ## 23    34     6       ASD            White      M 53.77   NA          44
    ## 24    20     6       ASD            White      M 37.30   NA          43
    ## 25    40     6       ASD         Lebanese      M 46.40   NA          41
    ## 26    36     6        TD            White      M 38.53   NA          46
    ## 27     5     6       ASD            White      M 54.13   NA          40
    ## 28    27     6        TD            White      M 41.17   NA          43
    ## 29    28     6        TD            White      M 39.43   NA          39
    ## 30    54     6        TD            White      M 39.30   NA          41
    ## 31     3     6        TD            White      F 45.07   NA          45
    ## 32    52     6        TD            White      M 43.03   NA          47
    ## 33    31     6        TD            White      M 43.80   NA          45
    ## 34    15     6        TD            White      M 40.30   NA          40
    ## 35    51     6        TD            Asian      F 42.10   NA          46
    ## 36    53     6        TD            White      M 44.43   NA          45
    ## 37    48     6       ASD            White      M    NA   NA          32
    ## 38     6     6       ASD      Bangladeshi      F 46.53   NA          39
    ## 39    19     6       ASD            White      M 56.73   NA          30
    ## 40    23     6       ASD     White/Latino      M 47.50   NA          30
    ## 41    61     6       ASD            White      M 62.33   NA          41
    ## 42    29     6       ASD            White      M 55.17   NA          39
    ## 43     8     6        TD            White      M 40.17   NA          43
    ## 44    30     6       ASD            White      M 56.43   NA          49
    ## 45    10     6        TD            White      M 40.43   NA          33
    ## 46    24     6       ASD            White      M 62.40   NA          30
    ## 47    46     6       ASD            White      M 57.43   NA          32
    ## 48    39     6       ASD      White/Asian      M 53.63   NA          30
    ## 49    32     6       ASD            White      M 54.63   NA          28
    ## 50    33     6       ASD African American      F 46.17   NA          33
    ## 51    57     6       ASD            White      F 61.70   NA          32
    ## 52    47     6        TD            White      F 40.37   NA          43
    ## 53    17     6       ASD            White      M 54.43   NA          27
    ## 54    56     6       ASD            White      M    NA   NA          34
    ## 55    50     6       ASD            White      M 62.40   NA          24
    ## 56    21     6       ASD African American      M 48.97   NA          26
    ## 57    26     5       ASD            White      M 46.93    4          NA
    ## 58    25     5        TD            White      M 37.93    0          NA
    ## 59    43     5       ASD            White      M 53.40   11          NA
    ## 60    13     5        TD            White      M 35.37    0          NA
    ## 61    12     5        TD            White      M 35.87    0          NA
    ## 62    42     5        TD            White      M 36.40    0          NA
    ## 63    37     5       ASD            White      M 55.17    4          NA
    ## 64    53     5        TD            White      M 40.07    0          NA
    ## 65    38     5        TD            White      F 36.13    0          NA
    ## 66    20     5       ASD            White      M 34.13    8          NA
    ## 67    18     5        TD            White      M 38.70    0          NA
    ## 68    14     5        TD            White      M 35.10    0          NA
    ## 69    35     5        TD            White      M 34.93    2          NA
    ## 70    54     5        TD            White      M 35.17    0          NA
    ## 71    52     5        TD            White      M 38.60    0          NA
    ## 72    31     5        TD            White      M 39.53    0          NA
    ## 73    22     5        TD            White      F 35.13    0          NA
    ## 74    41     5        TD            White      M 35.67    0          NA
    ## 75    59     5        TD            White      M 37.34    5          NA
    ## 76    28     5        TD            White      M 36.43    0          NA
    ## 77     3     5        TD            White      F 39.47    0          NA
    ## 78    49     5        TD            White      M 39.47    0          NA
    ## 79    16     5        TD            White      M 38.17    0          NA
    ## 80     4     5       ASD     White/Latino      M 47.40    9          NA
    ## 81    34     5       ASD            White      M 50.30   16          NA
    ## 82    58     5       ASD            White      M 42.30   16          NA
    ## 83    11     5        TD            White      M 35.63    0          NA
    ## 84    61     5       ASD            White      M 58.57   15          NA
    ## 85    45     5        TD            White      M 36.70   10          NA
    ## 86     1     5        TD            White      M 35.90    0          NA
    ## 87    36     5        TD            White      M 34.40    2          NA
    ## 88    44     5        TD            White      M 35.83    1          NA
    ## 89    10     5        TD            White      M 36.53    5          NA
    ## 90     5     5       ASD            White      M 49.30    9          NA
    ## 91    60     5       ASD            White      M 51.13   15          NA
    ## 92     6     5       ASD      Bangladeshi      F 42.43    8          NA
    ## 93     9     5        TD            White      F 35.00    1          NA
    ## 94    39     5       ASD      White/Asian      M 48.63   11          NA
    ## 95     8     5        TD            White      M 36.07    5          NA
    ## 96    27     5        TD            White      M 37.73    0          NA
    ## 97    47     5        TD            White      F 39.10   NA          NA
    ## 98    48     5       ASD            White      M 52.77   17          NA
    ## 99    51     5        TD            Asian      F 37.67    3          NA
    ## 100   19     5       ASD            White      M 51.97    9          NA
    ## 101   23     5       ASD     White/Latino      M 44.93   14          NA
    ## 102    7     5       ASD            White      F 57.73   17          NA
    ## 103   30     5       ASD            White      M 52.90   15          NA
    ## 104   29     5       ASD            White      M 50.97   20          NA
    ## 105   15     5        TD            White      M 35.90    3          NA
    ## 106   33     5       ASD African American      F 42.97   25          NA
    ## 107   24     5       ASD            White      M 57.20   17          NA
    ## 108   32     5       ASD            White      M 50.73   20          NA
    ## 109   46     5       ASD            White      M 54.33   17          NA
    ## 110   57     5       ASD            White      F 58.27   13          NA
    ## 111   17     5       ASD            White      M 50.93   15          NA
    ## 112   21     5       ASD African American      M 44.70   19          NA
    ## 113   50     5       ASD            White      M 57.20   17          NA
    ## 114   25     4        TD            White      M 34.00   NA          47
    ## 115   18     4        TD            White      M 34.43   NA          50
    ## 116   55     4        TD            White      M 32.03   NA          40
    ## 117   53     4        TD            White      M 36.40   NA          45
    ## 118   47     4        TD            White      F 32.13   NA          42
    ## 119   43     4       ASD            White      M 49.30   NA          50
    ## 120   42     4        TD            White      M 32.07   NA          39
    ## 121   16     4        TD            White      M 34.27   NA          45
    ## 122    3     4        TD            White      F 35.53   NA          39
    ## 123   31     4        TD            White      M 35.03   NA          43
    ## 124   20     4       ASD            White      M 30.80   NA          43
    ## 125   26     4       ASD            White      M 42.63   NA          41
    ## 126   54     4        TD            White      M 30.83   NA          39
    ## 127   60     4       ASD            White      M 47.00   NA          47
    ## 128   49     4        TD            White      M 35.63   NA          45
    ## 129    2     4       ASD            White      M 41.07   NA          49
    ## 130   52     4        TD            White      M 35.03   NA          45
    ## 131   59     4        TD            White      M 33.60   NA          34
    ## 132    5     4       ASD            White      M 45.53   NA          38
    ## 133   22     4        TD            White      F 30.93   NA          40
    ## 134   38     4        TD            White      F 32.17   NA          44
    ## 135   58     4       ASD            White      M 38.20   NA          40
    ## 136   14     4        TD            White      M 31.47   NA          29
    ## 137   13     4        TD            White      M 31.03   NA          40
    ## 138   37     4       ASD            White      M 51.67   NA          49
    ## 139   28     4        TD            White      M 31.00   NA          28
    ## 140   15     4        TD            White      M 31.27   NA          33
    ## 141    7     4       ASD            White      F 52.27   NA          29
    ## 142    8     4        TD            White      M 32.07   NA          42
    ## 143   40     4       ASD         Lebanese      M    NA   NA          NA
    ## 144    4     4       ASD     White/Latino      M 43.13   NA          41
    ## 145   36     4        TD            White      M 30.40   NA          35
    ## 146   19     4       ASD            White      M 47.83   NA          35
    ## 147   34     4       ASD            White      M 46.13   NA          31
    ## 148   41     4        TD            White      M 32.07   NA          41
    ## 149   45     4        TD            White      M 32.77   NA          26
    ## 150   10     4        TD            White      M 32.43   NA          31
    ## 151    9     4        TD            White      F 31.07   NA          45
    ## 152   27     4        TD            White      M 33.30   NA          42
    ## 153   44     4        TD            White      M 32.07   NA          40
    ## 154   11     4        TD            White      M 31.20   NA          35
    ## 155   51     4        TD            Asian      F 34.43   NA          30
    ## 156   61     4       ASD            White      M 54.13   NA          28
    ## 157    1     4        TD            White      M 32.90   NA          33
    ## 158   12     4        TD            White      M 32.07   NA          42
    ## 159    6     4       ASD      Bangladeshi      F 38.47   NA          33
    ## 160   23     4       ASD     White/Latino      M 42.23   NA          29
    ## 161   24     4       ASD            White      M 52.13   NA          29
    ## 162   33     4       ASD African American      F 36.93   NA          36
    ## 163   39     4       ASD      White/Asian      M 45.33   NA          45
    ## 164   21     4       ASD African American      M 40.27   NA          27
    ## 165   30     4       ASD            White      M 48.10   NA          47
    ## 166   35     4        TD            White      M 30.80   NA          36
    ## 167   50     4       ASD            White      M 52.13   NA          22
    ## 168   46     4       ASD            White      M 50.23   NA          29
    ## 169   48     4       ASD            White      M 43.63   NA          33
    ## 170   29     4       ASD            White      M 46.63   NA          27
    ## 171   32     4       ASD            White      M 47.10   NA          27
    ## 172   57     4       ASD            White      F 53.90   NA          30
    ## 173   26     3       ASD            White      M 38.60   NA          NA
    ## 174   31     3        TD            White      M 32.07   NA          NA
    ## 175   43     3       ASD            White      M 46.23   NA          NA
    ## 176    2     3       ASD            White      M 37.07   NA          NA
    ## 177   18     3        TD            White      M 30.03   NA          NA
    ## 178   52     3        TD            White      M 30.77   NA          NA
    ## 179   25     3        TD            White      M 30.13   NA          NA
    ## 180   16     3        TD            White      M 30.63   NA          NA
    ## 181   38     3        TD            White      F 28.07   NA          NA
    ## 182   12     3        TD            White      M 28.27   NA          NA
    ## 183   35     3        TD            White      M 27.13   NA          NA
    ## 184   60     3       ASD            White      M 42.47   NA          NA
    ## 185   37     3       ASD            White      M 47.73   NA          NA
    ## 186   55     3        TD            White      M 28.03   NA          NA
    ## 187   49     3        TD            White      M 31.63   NA          NA
    ## 188   53     3        TD            White      M 32.50   NA          NA
    ## 189   13     3        TD            White      M 27.07   NA          NA
    ## 190   42     3        TD            White      M 28.90   NA          NA
    ## 191    3     3        TD            White      F 31.83   NA          NA
    ## 192   44     3        TD            White      M 27.67   NA          NA
    ## 193   20     3       ASD            White      M 26.77   NA          NA
    ## 194    4     3       ASD     White/Latino      M 38.90   NA          NA
    ## 195   47     3        TD            White      F 29.53   NA          NA
    ## 196    5     3       ASD            White      M 41.57   NA          NA
    ## 197    9     3        TD            White      F 26.63   NA          NA
    ## 198   59     3        TD            White      M 29.67   NA          NA
    ## 199   40     3       ASD         Lebanese      M 34.00   NA          NA
    ## 200   28     3        TD            White      M 27.03   NA          NA
    ## 201   58     3       ASD            White      M 34.27   NA          NA
    ## 202   14     3        TD            White      M 26.80   NA          NA
    ## 203   34     3       ASD            White      M 43.00   NA          NA
    ## 204   45     3        TD            White      M 28.63   NA          NA
    ## 205   41     3        TD            White      M 28.47   NA          NA
    ## 206   19     3       ASD            White      M 43.90   NA          NA
    ## 207   22     3        TD            White      F 26.67   NA          NA
    ## 208   27     3        TD            White      M 29.07   NA          NA
    ## 209   54     3        TD            White      M 27.07   NA          NA
    ## 210    1     3        TD            White      M 27.70   NA          NA
    ## 211   61     3       ASD            White      M 50.63   NA          NA
    ## 212   15     3        TD            White      M 28.10   NA          NA
    ## 213   39     3       ASD      White/Asian      M 41.73   NA          NA
    ## 214   36     3        TD            White      M 26.87   NA          NA
    ## 215    7     3       ASD            White      F 49.20   NA          NA
    ## 216   11     3        TD            White      M 27.27   NA          NA
    ## 217   10     3        TD            White      M 28.53   NA          NA
    ## 218   30     3       ASD            White      M 44.67   NA          NA
    ## 219   23     3       ASD     White/Latino      M 35.93   NA          NA
    ## 220   51     3        TD            Asian      F 29.67   NA          NA
    ## 221   21     3       ASD African American      M 36.17   NA          NA
    ## 222   50     3       ASD            White      M 46.37   NA          NA
    ## 223    6     3       ASD      Bangledeshi      F 34.57   NA          NA
    ## 224   17     3       ASD            White      M    NA   NA          NA
    ## 225   57     3       ASD            White      F 49.97   NA          NA
    ## 226   24     3       ASD            White      M 46.37   NA          NA
    ## 227   29     3       ASD            White      M 43.10   NA          NA
    ## 228   33     3       ASD African American      F 34.10   NA          NA
    ## 229   56     3       ASD            White      M 44.47   NA          NA
    ## 230   46     3       ASD            White      M 45.57   NA          NA
    ## 231   32     3       ASD            White      M 42.13   NA          NA
    ## 232   35     2        TD            White      M 23.43   NA          NA
    ## 233   18     2        TD            White      M 26.13   NA          NA
    ## 234   16     2        TD            White      M 26.27   NA          NA
    ## 235   60     2       ASD            White      M 38.63   NA          NA
    ## 236   25     2        TD            White      M 25.47   NA          NA
    ## 237   47     2        TD            White      F 24.40   NA          NA
    ## 238    2     2       ASD            White      M 33.17   NA          NA
    ## 239    4     2       ASD     White/Latino      M 35.30   NA          NA
    ## 240    8     2        TD            White      M 24.07   NA          NA
    ## 241   31     2        TD            White      M 27.70   NA          NA
    ## 242   43     2       ASD            White      M 42.77   NA          NA
    ## 243    3     2        TD            White      F 27.83   NA          NA
    ## 244    5     2       ASD            White      M 37.57   NA          NA
    ## 245   53     2        TD            White      M 28.60   NA          NA
    ## 246   38     2        TD            White      F 24.17   NA          NA
    ## 247   37     2       ASD            White      M 43.68   NA          NA
    ## 248   42     2        TD            White      M    NA   NA          NA
    ## 249   22     2        TD            White      F 22.90   NA          NA
    ## 250   59     2        TD            White      M 25.87   NA          NA
    ## 251   13     2        TD            White      M 22.97   NA          NA
    ## 252   34     2       ASD            White      M 38.13   NA          NA
    ## 253   20     2       ASD            White      M 22.50   NA          NA
    ## 254   49     2        TD            White      M 27.47   NA          NA
    ## 255   61     2       ASD            White      M 46.77   NA          NA
    ## 256   19     2       ASD            White      M 39.93   NA          NA
    ## 257   52     2        TD            White      M 26.63   NA          NA
    ## 258   14     2        TD            White      M 22.87   NA          NA
    ## 259   54     2        TD            White      M 23.17   NA          NA
    ## 260   41     2        TD            White      M 23.07   NA          NA
    ## 261   58     2       ASD            White      M 30.10   NA          NA
    ## 262   28     2        TD            White      M 23.57   NA          NA
    ## 263   55     2        TD            White      M 23.93   NA          NA
    ## 264   27     2        TD            White      M 25.23   NA          NA
    ## 265   12     2        TD            White      M 23.83   NA          NA
    ## 266   29     2       ASD            White      M 38.13   NA          NA
    ## 267    7     2       ASD            White      F 49.20   NA          NA
    ## 268   45     2        TD            White      M 24.68   NA          NA
    ## 269   15     2        TD            White      M 24.80   NA          NA
    ## 270   23     2       ASD     White/Latino      M 32.10   NA          NA
    ## 271    6     2       ASD      Bangladeshi      F 30.27   NA          NA
    ## 272    1     2        TD            White      M 23.93   NA          NA
    ## 273   40     2       ASD         Lebanese      M 32.20   NA          NA
    ## 274   50     2       ASD            White      M 42.20   NA          NA
    ## 275    9     2        TD            White      F 23.73   NA          NA
    ## 276   39     2       ASD      White/Asian      M 37.37   NA          NA
    ## 277   44     2        TD            White      M 23.63   NA          NA
    ## 278   11     2        TD            White      M    NA   NA          NA
    ## 279   30     2       ASD            White      M 40.50   NA          NA
    ## 280   36     2        TD            White      M 22.87   NA          NA
    ## 281   51     2        TD            Asian      F 25.40   NA          NA
    ## 282   17     2       ASD            White      M 38.73   NA          NA
    ## 283   21     2       ASD African American      M 31.80   NA          NA
    ## 284   10     2        TD            White      M 23.83   NA          NA
    ## 285   57     2       ASD            White      F 41.07   NA          NA
    ## 286   46     2       ASD            White      M 41.50   NA          NA
    ## 287   24     2       ASD            White      M 42.20   NA          NA
    ## 288   56     2       ASD            White      M 39.40   NA          NA
    ## 289   33     2       ASD African American      F 29.70   NA          NA
    ## 290   32     2       ASD            White      M 38.33   NA          NA
    ## 291   48     2       ASD            White      M 37.20   NA          NA
    ## 292   26     1       ASD            White      M 30.40   11          32
    ## 293   53     1        TD            White      M 23.90    0          30
    ## 294   60     1       ASD            White      M 34.00   13          30
    ## 295    2     1       ASD            White      M 28.80   13          34
    ## 296   43     1       ASD            White      M 37.03   14          42
    ## 297   25     1        TD            White      M 21.77    0          29
    ## 298   18     1        TD            White      M 20.77    0          29
    ## 299   38     1        TD            White      F 19.87    1          29
    ## 300   37     1       ASD            White      M 39.50    7          33
    ## 301   31     1        TD            White      M 23.13    0          27
    ## 302   52     1        TD            White      M 22.57    0          29
    ## 303   49     1        TD            White      M 23.07    0          27
    ## 304    4     1       ASD     White/Latino      M 31.03    8          31
    ## 305    3     1        TD            White      F 23.50    1          29
    ## 306   35     1        TD            White      M 19.30    1          23
    ## 307   16     1        TD            White      M 21.67    0          29
    ## 308   13     1        TD            White      M 19.00    0          25
    ## 309   59     1        TD            White      M 20.80    4          29
    ## 310   41     1        TD            White      M 19.23    0          25
    ## 311   58     1       ASD            White      M 26.00   15          30
    ## 312   19     1       ASD            White      M 35.80   11          28
    ## 313   20     1       ASD            White      M 18.77    9          26
    ## 314   47     1        TD            White      F 20.03    0          27
    ## 315    5     1       ASD            White      M 34.03    9          34
    ## 316   34     1       ASD            White      M 33.77   10          27
    ## 317   42     1        TD            White      M 19.37    0          24
    ## 318   61     1       ASD            White      M 42.00   15          27
    ## 319   55     1        TD            White      M 19.97    0          26
    ## 320    8     1        TD            White      M 20.10    5          32
    ## 321    1     1        TD            White      M 19.80    0          28
    ## 322    9     1        TD            White      F 18.30    0          24
    ## 323    7     1       ASD            White      F 41.00   18          24
    ## 324   21     1       ASD African American      M 27.53   21          22
    ## 325   22     1        TD            White      F 18.93    0          21
    ## 326   29     1       ASD            White      M 34.87   17          26
    ## 327   28     1        TD            White      M 19.93    0          20
    ## 328   39     1       ASD      White/Asian      M 33.20   11          26
    ## 329   54     1        TD            White      M 19.10    1          24
    ## 330   14     1        TD            White      M 18.97    0          23
    ## 331   23     1       ASD     White/Latino      M 27.37   14          25
    ## 332   11     1        TD            White      M 19.23    0          21
    ## 333   15     1        TD            White      M 19.27    0          24
    ## 334   27     1        TD            White      M 21.03    0          26
    ## 335   45     1        TD            White      M 20.03    5          24
    ## 336   10     1        TD            White      M 19.27    3          27
    ## 337   44     1        TD            White      M 19.77    3          30
    ## 338   46     1       ASD            White      M 36.73   20          21
    ## 339   12     1        TD            White      M 20.07    0          30
    ## 340   40     1       ASD         Lebanese      M 24.90   13          27
    ## 341   56     1       ASD            White      M 35.50   14          27
    ## 342   50     1       ASD            White      M 37.47   19          17
    ## 343   48     1       ASD            White      M 31.63   17          28
    ## 344   51     1        TD            Asian      F 20.87    1          22
    ## 345   17     1       ASD            White      M 34.80   14          25
    ## 346   33     1       ASD African American      F 25.33   14          25
    ## 347    6     1       ASD      Bangladeshi      F 26.17   17          20
    ## 348   24     1       ASD            White      M 37.47   20          13
    ## 349   30     1       ASD            White      M 36.53   12          31
    ## 350   36     1        TD            White      M 19.20    3          19
    ## 351   32     1       ASD            White      M 34.27   21          21
    ## 352   57     1       ASD            White      F 41.07   15          28
...
``` r
#
arrange2 <- df_merged %>%  group_by(SUBJ) %>%  filter(VISIT == 1) %>%  arrange(tokens_CHI)
arrange(df_merged, VISIT, tokens_CHI)
```

    ##     SUBJ VISIT Diagnosis        Ethnicity Gender   Age ADOS nonVerbalIQ
    ## 1     57     1       ASD            White      F 41.07   15          28
    ## 2     32     1       ASD            White      M 34.27   21          21
    ## 3     36     1        TD            White      M 19.20    3          19
    ## 4     30     1       ASD            White      M 36.53   12          31
    ## 5     24     1       ASD            White      M 37.47   20          13
    ## 6      6     1       ASD      Bangladeshi      F 26.17   17          20
    ## 7     33     1       ASD African American      F 25.33   14          25
    ## 8     17     1       ASD            White      M 34.80   14          25
    ## 9     51     1        TD            Asian      F 20.87    1          22
    ## 10    48     1       ASD            White      M 31.63   17          28
    ## 11    50     1       ASD            White      M 37.47   19          17
    ## 12    56     1       ASD            White      M 35.50   14          27
    ## 13    40     1       ASD         Lebanese      M 24.90   13          27
    ## 14    12     1        TD            White      M 20.07    0          30
    ## 15    46     1       ASD            White      M 36.73   20          21
    ## 16    44     1        TD            White      M 19.77    3          30
    ## 17    10     1        TD            White      M 19.27    3          27
    ## 18    45     1        TD            White      M 20.03    5          24
    ## 19    27     1        TD            White      M 21.03    0          26
    ## 20    15     1        TD            White      M 19.27    0          24
    ## 21    11     1        TD            White      M 19.23    0          21
    ## 22    23     1       ASD     White/Latino      M 27.37   14          25
    ## 23    14     1        TD            White      M 18.97    0          23
    ## 24    39     1       ASD      White/Asian      M 33.20   11          26
    ## 25    54     1        TD            White      M 19.10    1          24
    ## 26    28     1        TD            White      M 19.93    0          20
    ## 27    29     1       ASD            White      M 34.87   17          26
    ## 28    22     1        TD            White      F 18.93    0          21
    ## 29    21     1       ASD African American      M 27.53   21          22
    ## 30     7     1       ASD            White      F 41.00   18          24
    ## 31     9     1        TD            White      F 18.30    0          24
    ## 32     1     1        TD            White      M 19.80    0          28
    ## 33     8     1        TD            White      M 20.10    5          32
    ## 34    55     1        TD            White      M 19.97    0          26
    ## 35    61     1       ASD            White      M 42.00   15          27
    ## 36    42     1        TD            White      M 19.37    0          24
    ## 37    34     1       ASD            White      M 33.77   10          27
    ## 38     5     1       ASD            White      M 34.03    9          34
    ## 39    47     1        TD            White      F 20.03    0          27
    ## 40    20     1       ASD            White      M 18.77    9          26
    ## 41    19     1       ASD            White      M 35.80   11          28
    ## 42    58     1       ASD            White      M 26.00   15          30
    ## 43    41     1        TD            White      M 19.23    0          25
    ## 44    59     1        TD            White      M 20.80    4          29
    ## 45    13     1        TD            White      M 19.00    0          25
    ## 46    16     1        TD            White      M 21.67    0          29
    ## 47     3     1        TD            White      F 23.50    1          29
    ## 48    35     1        TD            White      M 19.30    1          23
    ## 49     4     1       ASD     White/Latino      M 31.03    8          31
    ## 50    49     1        TD            White      M 23.07    0          27
    ## 51    31     1        TD            White      M 23.13    0          27
    ## 52    52     1        TD            White      M 22.57    0          29
    ## 53    37     1       ASD            White      M 39.50    7          33
    ## 54    38     1        TD            White      F 19.87    1          29
    ## 55    18     1        TD            White      M 20.77    0          29
    ## 56    25     1        TD            White      M 21.77    0          29
    ## 57    43     1       ASD            White      M 37.03   14          42
    ## 58     2     1       ASD            White      M 28.80   13          34
    ## 59    60     1       ASD            White      M 34.00   13          30
    ## 60    53     1        TD            White      M 23.90    0          30
    ## 61    26     1       ASD            White      M 30.40   11          32
    ## 62    48     2       ASD            White      M 37.20   NA          NA
    ## 63    32     2       ASD            White      M 38.33   NA          NA
    ## 64    33     2       ASD African American      F 29.70   NA          NA
    ## 65    56     2       ASD            White      M 39.40   NA          NA
    ## 66    24     2       ASD            White      M 42.20   NA          NA
    ## 67    46     2       ASD            White      M 41.50   NA          NA
    ## 68    57     2       ASD            White      F 41.07   NA          NA
    ## 69    10     2        TD            White      M 23.83   NA          NA
    ## 70    21     2       ASD African American      M 31.80   NA          NA
    ## 71    17     2       ASD            White      M 38.73   NA          NA
    ## 72    51     2        TD            Asian      F 25.40   NA          NA
    ## 73    36     2        TD            White      M 22.87   NA          NA
    ## 74    30     2       ASD            White      M 40.50   NA          NA
    ## 75    11     2        TD            White      M    NA   NA          NA
    ## 76    44     2        TD            White      M 23.63   NA          NA
    ## 77    39     2       ASD      White/Asian      M 37.37   NA          NA
    ## 78     9     2        TD            White      F 23.73   NA          NA
    ## 79    50     2       ASD            White      M 42.20   NA          NA
    ## 80     1     2        TD            White      M 23.93   NA          NA
    ## 81    40     2       ASD         Lebanese      M 32.20   NA          NA
    ## 82     6     2       ASD      Bangladeshi      F 30.27   NA          NA
    ## 83    23     2       ASD     White/Latino      M 32.10   NA          NA
    ## 84    15     2        TD            White      M 24.80   NA          NA
    ## 85    45     2        TD            White      M 24.68   NA          NA
    ## 86     7     2       ASD            White      F 49.20   NA          NA
    ## 87    29     2       ASD            White      M 38.13   NA          NA
    ## 88    12     2        TD            White      M 23.83   NA          NA
    ## 89    27     2        TD            White      M 25.23   NA          NA
    ## 90    55     2        TD            White      M 23.93   NA          NA
    ## 91    28     2        TD            White      M 23.57   NA          NA
    ## 92    58     2       ASD            White      M 30.10   NA          NA
    ## 93    41     2        TD            White      M 23.07   NA          NA
    ## 94    54     2        TD            White      M 23.17   NA          NA
    ## 95    14     2        TD            White      M 22.87   NA          NA
    ## 96    52     2        TD            White      M 26.63   NA          NA
    ## 97    19     2       ASD            White      M 39.93   NA          NA
    ## 98    49     2        TD            White      M 27.47   NA          NA
    ## 99    61     2       ASD            White      M 46.77   NA          NA
    ## 100   20     2       ASD            White      M 22.50   NA          NA
    ## 101   34     2       ASD            White      M 38.13   NA          NA
    ## 102   13     2        TD            White      M 22.97   NA          NA
    ## 103   59     2        TD            White      M 25.87   NA          NA
    ## 104   22     2        TD            White      F 22.90   NA          NA
    ## 105   42     2        TD            White      M    NA   NA          NA
    ## 106   37     2       ASD            White      M 43.68   NA          NA
    ## 107   38     2        TD            White      F 24.17   NA          NA
    ## 108   53     2        TD            White      M 28.60   NA          NA
    ## 109    5     2       ASD            White      M 37.57   NA          NA
    ## 110    3     2        TD            White      F 27.83   NA          NA
    ## 111   43     2       ASD            White      M 42.77   NA          NA
    ## 112   31     2        TD            White      M 27.70   NA          NA
    ## 113    8     2        TD            White      M 24.07   NA          NA
    ## 114    4     2       ASD     White/Latino      M 35.30   NA          NA
    ## 115    2     2       ASD            White      M 33.17   NA          NA
    ## 116   47     2        TD            White      F 24.40   NA          NA
    ## 117   25     2        TD            White      M 25.47   NA          NA
    ## 118   60     2       ASD            White      M 38.63   NA          NA
    ## 119   16     2        TD            White      M 26.27   NA          NA
    ## 120   18     2        TD            White      M 26.13   NA          NA
    ## 121   35     2        TD            White      M 23.43   NA          NA
    ## 122   32     3       ASD            White      M 42.13   NA          NA
    ## 123   46     3       ASD            White      M 45.57   NA          NA
    ## 124   56     3       ASD            White      M 44.47   NA          NA
    ## 125   33     3       ASD African American      F 34.10   NA          NA
    ## 126   29     3       ASD            White      M 43.10   NA          NA
    ## 127   24     3       ASD            White      M 46.37   NA          NA
    ## 128   57     3       ASD            White      F 49.97   NA          NA
    ## 129   17     3       ASD            White      M    NA   NA          NA
    ## 130    6     3       ASD      Bangledeshi      F 34.57   NA          NA
    ## 131   50     3       ASD            White      M 46.37   NA          NA
    ## 132   21     3       ASD African American      M 36.17   NA          NA
    ## 133   51     3        TD            Asian      F 29.67   NA          NA
    ## 134   23     3       ASD     White/Latino      M 35.93   NA          NA
    ## 135   30     3       ASD            White      M 44.67   NA          NA
    ## 136   10     3        TD            White      M 28.53   NA          NA
    ## 137   11     3        TD            White      M 27.27   NA          NA
    ## 138    7     3       ASD            White      F 49.20   NA          NA
    ## 139   36     3        TD            White      M 26.87   NA          NA
    ## 140   39     3       ASD      White/Asian      M 41.73   NA          NA
    ## 141   15     3        TD            White      M 28.10   NA          NA
    ## 142   61     3       ASD            White      M 50.63   NA          NA
    ## 143    1     3        TD            White      M 27.70   NA          NA
    ## 144   54     3        TD            White      M 27.07   NA          NA
    ## 145   27     3        TD            White      M 29.07   NA          NA
    ## 146   22     3        TD            White      F 26.67   NA          NA
    ## 147   19     3       ASD            White      M 43.90   NA          NA
    ## 148   41     3        TD            White      M 28.47   NA          NA
    ## 149   45     3        TD            White      M 28.63   NA          NA
    ## 150   34     3       ASD            White      M 43.00   NA          NA
    ## 151   14     3        TD            White      M 26.80   NA          NA
    ## 152   58     3       ASD            White      M 34.27   NA          NA
    ## 153   28     3        TD            White      M 27.03   NA          NA
    ## 154   40     3       ASD         Lebanese      M 34.00   NA          NA
    ## 155   59     3        TD            White      M 29.67   NA          NA
    ## 156    9     3        TD            White      F 26.63   NA          NA
    ## 157    5     3       ASD            White      M 41.57   NA          NA
    ## 158   47     3        TD            White      F 29.53   NA          NA
    ## 159    4     3       ASD     White/Latino      M 38.90   NA          NA
    ## 160   20     3       ASD            White      M 26.77   NA          NA
    ## 161   44     3        TD            White      M 27.67   NA          NA
    ## 162    3     3        TD            White      F 31.83   NA          NA
    ## 163   42     3        TD            White      M 28.90   NA          NA
    ## 164   13     3        TD            White      M 27.07   NA          NA
    ## 165   53     3        TD            White      M 32.50   NA          NA
    ## 166   49     3        TD            White      M 31.63   NA          NA
    ## 167   55     3        TD            White      M 28.03   NA          NA
    ## 168   37     3       ASD            White      M 47.73   NA          NA
    ## 169   60     3       ASD            White      M 42.47   NA          NA
    ## 170   35     3        TD            White      M 27.13   NA          NA
    ## 171   12     3        TD            White      M 28.27   NA          NA
    ## 172   16     3        TD            White      M 30.63   NA          NA
    ## 173   38     3        TD            White      F 28.07   NA          NA
    ## 174   25     3        TD            White      M 30.13   NA          NA
    ## 175   52     3        TD            White      M 30.77   NA          NA
    ## 176   18     3        TD            White      M 30.03   NA          NA
    ## 177    2     3       ASD            White      M 37.07   NA          NA
    ## 178   43     3       ASD            White      M 46.23   NA          NA
    ## 179   31     3        TD            White      M 32.07   NA          NA
    ## 180   26     3       ASD            White      M 38.60   NA          NA
    ## 181   57     4       ASD            White      F 53.90   NA          30
    ## 182   32     4       ASD            White      M 47.10   NA          27
    ## 183   29     4       ASD            White      M 46.63   NA          27
    ## 184   48     4       ASD            White      M 43.63   NA          33
    ## 185   46     4       ASD            White      M 50.23   NA          29
    ## 186   50     4       ASD            White      M 52.13   NA          22
    ## 187   35     4        TD            White      M 30.80   NA          36
    ## 188   30     4       ASD            White      M 48.10   NA          47
    ## 189   21     4       ASD African American      M 40.27   NA          27
    ## 190   39     4       ASD      White/Asian      M 45.33   NA          45
    ## 191   33     4       ASD African American      F 36.93   NA          36
    ## 192   24     4       ASD            White      M 52.13   NA          29
    ## 193   23     4       ASD     White/Latino      M 42.23   NA          29
    ## 194    6     4       ASD      Bangladeshi      F 38.47   NA          33
    ## 195   12     4        TD            White      M 32.07   NA          42
    ## 196    1     4        TD            White      M 32.90   NA          33
    ## 197   61     4       ASD            White      M 54.13   NA          28
    ## 198   51     4        TD            Asian      F 34.43   NA          30
    ## 199   11     4        TD            White      M 31.20   NA          35
    ## 200   44     4        TD            White      M 32.07   NA          40
    ## 201   27     4        TD            White      M 33.30   NA          42
    ## 202    9     4        TD            White      F 31.07   NA          45
    ## 203   10     4        TD            White      M 32.43   NA          31
    ## 204   45     4        TD            White      M 32.77   NA          26
    ## 205   41     4        TD            White      M 32.07   NA          41
    ## 206   34     4       ASD            White      M 46.13   NA          31
    ## 207   19     4       ASD            White      M 47.83   NA          35
    ## 208   36     4        TD            White      M 30.40   NA          35
    ## 209    4     4       ASD     White/Latino      M 43.13   NA          41
    ## 210    8     4        TD            White      M 32.07   NA          42
    ## 211   40     4       ASD         Lebanese      M    NA   NA          NA
    ## 212    7     4       ASD            White      F 52.27   NA          29
    ## 213   15     4        TD            White      M 31.27   NA          33
    ## 214   28     4        TD            White      M 31.00   NA          28
    ## 215   37     4       ASD            White      M 51.67   NA          49
    ## 216   13     4        TD            White      M 31.03   NA          40
    ## 217   14     4        TD            White      M 31.47   NA          29
    ## 218   58     4       ASD            White      M 38.20   NA          40
    ## 219   38     4        TD            White      F 32.17   NA          44
    ## 220   22     4        TD            White      F 30.93   NA          40
    ## 221    5     4       ASD            White      M 45.53   NA          38
    ## 222   59     4        TD            White      M 33.60   NA          34
    ## 223   52     4        TD            White      M 35.03   NA          45
    ## 224    2     4       ASD            White      M 41.07   NA          49
    ## 225   49     4        TD            White      M 35.63   NA          45
    ## 226   60     4       ASD            White      M 47.00   NA          47
    ## 227   54     4        TD            White      M 30.83   NA          39
    ## 228   26     4       ASD            White      M 42.63   NA          41
    ## 229   20     4       ASD            White      M 30.80   NA          43
    ## 230   31     4        TD            White      M 35.03   NA          43
    ## 231    3     4        TD            White      F 35.53   NA          39
    ## 232   16     4        TD            White      M 34.27   NA          45
    ## 233   42     4        TD            White      M 32.07   NA          39
    ## 234   43     4       ASD            White      M 49.30   NA          50
    ## 235   47     4        TD            White      F 32.13   NA          42
    ## 236   53     4        TD            White      M 36.40   NA          45
    ## 237   55     4        TD            White      M 32.03   NA          40
    ## 238   18     4        TD            White      M 34.43   NA          50
    ## 239   25     4        TD            White      M 34.00   NA          47
    ## 240   50     5       ASD            White      M 57.20   17          NA
    ## 241   21     5       ASD African American      M 44.70   19          NA
    ## 242   17     5       ASD            White      M 50.93   15          NA
    ## 243   57     5       ASD            White      F 58.27   13          NA
    ## 244   46     5       ASD            White      M 54.33   17          NA
    ## 245   32     5       ASD            White      M 50.73   20          NA
    ## 246   24     5       ASD            White      M 57.20   17          NA
    ## 247   33     5       ASD African American      F 42.97   25          NA
    ## 248   15     5        TD            White      M 35.90    3          NA
    ## 249   29     5       ASD            White      M 50.97   20          NA
    ## 250   30     5       ASD            White      M 52.90   15          NA
    ## 251    7     5       ASD            White      F 57.73   17          NA
    ## 252   19     5       ASD            White      M 51.97    9          NA
    ## 253   23     5       ASD     White/Latino      M 44.93   14          NA
    ## 254   51     5        TD            Asian      F 37.67    3          NA
    ## 255   48     5       ASD            White      M 52.77   17          NA
    ## 256   47     5        TD            White      F 39.10   NA          NA
    ## 257   27     5        TD            White      M 37.73    0          NA
    ## 258    8     5        TD            White      M 36.07    5          NA
    ## 259   39     5       ASD      White/Asian      M 48.63   11          NA
    ## 260    9     5        TD            White      F 35.00    1          NA
    ## 261    6     5       ASD      Bangladeshi      F 42.43    8          NA
    ## 262   60     5       ASD            White      M 51.13   15          NA
    ## 263    5     5       ASD            White      M 49.30    9          NA
    ## 264   10     5        TD            White      M 36.53    5          NA
    ## 265   44     5        TD            White      M 35.83    1          NA
    ## 266   36     5        TD            White      M 34.40    2          NA
    ## 267    1     5        TD            White      M 35.90    0          NA
    ## 268   45     5        TD            White      M 36.70   10          NA
    ## 269   61     5       ASD            White      M 58.57   15          NA
    ## 270   11     5        TD            White      M 35.63    0          NA
    ## 271   58     5       ASD            White      M 42.30   16          NA
    ## 272   34     5       ASD            White      M 50.30   16          NA
    ## 273    4     5       ASD     White/Latino      M 47.40    9          NA
    ## 274   16     5        TD            White      M 38.17    0          NA
    ## 275   49     5        TD            White      M 39.47    0          NA
    ## 276    3     5        TD            White      F 39.47    0          NA
    ## 277   28     5        TD            White      M 36.43    0          NA
    ## 278   59     5        TD            White      M 37.34    5          NA
    ## 279   41     5        TD            White      M 35.67    0          NA
    ## 280   22     5        TD            White      F 35.13    0          NA
    ## 281   31     5        TD            White      M 39.53    0          NA
    ## 282   52     5        TD            White      M 38.60    0          NA
    ## 283   54     5        TD            White      M 35.17    0          NA
    ## 284   35     5        TD            White      M 34.93    2          NA
    ## 285   14     5        TD            White      M 35.10    0          NA
    ## 286   18     5        TD            White      M 38.70    0          NA
    ## 287   20     5       ASD            White      M 34.13    8          NA
    ## 288   38     5        TD            White      F 36.13    0          NA
    ## 289   53     5        TD            White      M 40.07    0          NA
    ## 290   37     5       ASD            White      M 55.17    4          NA
    ## 291   42     5        TD            White      M 36.40    0          NA
    ## 292   12     5        TD            White      M 35.87    0          NA
    ## 293   13     5        TD            White      M 35.37    0          NA
    ## 294   43     5       ASD            White      M 53.40   11          NA
    ## 295   25     5        TD            White      M 37.93    0          NA
    ## 296   26     5       ASD            White      M 46.93    4          NA
    ## 297   21     6       ASD African American      M 48.97   NA          26
    ## 298   50     6       ASD            White      M 62.40   NA          24
    ## 299   56     6       ASD            White      M    NA   NA          34
    ## 300   17     6       ASD            White      M 54.43   NA          27
    ## 301   47     6        TD            White      F 40.37   NA          43
    ## 302   57     6       ASD            White      F 61.70   NA          32
    ## 303   33     6       ASD African American      F 46.17   NA          33
    ## 304   32     6       ASD            White      M 54.63   NA          28
    ## 305   39     6       ASD      White/Asian      M 53.63   NA          30
    ## 306   46     6       ASD            White      M 57.43   NA          32
    ## 307   24     6       ASD            White      M 62.40   NA          30
    ## 308   10     6        TD            White      M 40.43   NA          33
    ## 309   30     6       ASD            White      M 56.43   NA          49
    ## 310    8     6        TD            White      M 40.17   NA          43
    ## 311   29     6       ASD            White      M 55.17   NA          39
    ## 312   61     6       ASD            White      M 62.33   NA          41
    ## 313   23     6       ASD     White/Latino      M 47.50   NA          30
    ## 314   19     6       ASD            White      M 56.73   NA          30
    ## 315    6     6       ASD      Bangladeshi      F 46.53   NA          39
    ## 316   48     6       ASD            White      M    NA   NA          32
    ## 317   53     6        TD            White      M 44.43   NA          45
    ## 318   51     6        TD            Asian      F 42.10   NA          46
    ## 319   15     6        TD            White      M 40.30   NA          40
    ## 320   31     6        TD            White      M 43.80   NA          45
    ## 321   52     6        TD            White      M 43.03   NA          47
    ## 322    3     6        TD            White      F 45.07   NA          45
    ## 323   54     6        TD            White      M 39.30   NA          41
    ## 324   28     6        TD            White      M 39.43   NA          39
    ## 325   27     6        TD            White      M 41.17   NA          43
    ## 326    5     6       ASD            White      M 54.13   NA          40
    ## 327   36     6        TD            White      M 38.53   NA          46
    ## 328   40     6       ASD         Lebanese      M 46.40   NA          41
    ## 329   20     6       ASD            White      M 37.30   NA          43
    ## 330   34     6       ASD            White      M 53.77   NA          44
    ## 331   35     6        TD            White      M 39.07   NA          45
    ## 332   58     6       ASD            White      M 46.07   NA          45
    ## 333   12     6        TD            White      M 41.50   NA          44
    ## 334   49     6        TD            White      M 43.40   NA          45
    ## 335   59     6        TD            White      M 41.00   NA          NA
    ## 336   11     6        TD            White      M 40.27   NA          42
    ## 337    1     6        TD            White      M 40.13   NA          42
    ## 338    2     6       ASD            White      M 49.70   NA          48
    ## 339   41     6        TD            White      M 39.93   NA          45
    ## 340   16     6        TD            White      M 42.93   NA          49
    ## 341   60     6       ASD            White      M 54.73   NA          50
    ## 342   42     6        TD            White      M 40.13   NA          46
    ## 343    4     6       ASD     White/Latino      M 51.37   NA          44
    ## 344   14     6        TD            White      M 39.43   NA          44
    ## 345   22     6        TD            White      F 39.23   NA          43
    ## 346   13     6        TD            White      M 39.40   NA          47
    ## 347   37     6       ASD            White      M 58.77   NA          50
    ## 348   43     6       ASD            White      M 57.37   NA          49
    ## 349   25     6        TD            White      M 42.47   NA          48
    ## 350   18     6        TD            White      M 44.07   NA          50
    ## 351   26     6       ASD            White      M 51.00   NA          46
    ## 352   55     6        TD            White      M 41.93   NA          45
    ##     verbalIQ Socialization  MOT_MLU   CHI_MLU types_MOT types_CHI
...
``` r
#1
select1 <- df_merged %>% filter(Diagnosis == "ASD") %>% select(SUBJ, VISIT, CHI_MLU, MOT_MLU, tokens_CHI, tokens_MOT)

#2
select2 <- df_merged %>% filter(Diagnosis == "ASD") %>% select(SUBJ, SUBJ, VISIT, CHI_MLU, MOT_MLU, tokens_CHI, tokens_MOT)
#Nothing happens if you include the name of a variable twice in select()
```
...
summarise function and pipes to add a column in the data set containing
...
``` r
#1
mutate(df_merged, meanAllVisit = mean(tokens_CHI))
```

    ##     SUBJ VISIT Diagnosis        Ethnicity Gender   Age ADOS nonVerbalIQ
    ## 1      1     1        TD            White      M 19.80    0          28
    ## 2      1     2        TD            White      M 23.93   NA          NA
    ## 3      1     3        TD            White      M 27.70   NA          NA
    ## 4      1     4        TD            White      M 32.90   NA          33
    ## 5      1     5        TD            White      M 35.90    0          NA
    ## 6      1     6        TD            White      M 40.13   NA          42
    ## 7      2     1       ASD            White      M 28.80   13          34
    ## 8      2     2       ASD            White      M 33.17   NA          NA
    ## 9      2     3       ASD            White      M 37.07   NA          NA
    ## 10     2     4       ASD            White      M 41.07   NA          49
    ## 11     2     6       ASD            White      M 49.70   NA          48
    ## 12     3     1        TD            White      F 23.50    1          29
    ## 13     3     2        TD            White      F 27.83   NA          NA
    ## 14     3     3        TD            White      F 31.83   NA          NA
    ## 15     3     4        TD            White      F 35.53   NA          39
    ## 16     3     5        TD            White      F 39.47    0          NA
    ## 17     3     6        TD            White      F 45.07   NA          45
    ## 18     4     1       ASD     White/Latino      M 31.03    8          31
    ## 19     4     2       ASD     White/Latino      M 35.30   NA          NA
    ## 20     4     3       ASD     White/Latino      M 38.90   NA          NA
    ## 21     4     4       ASD     White/Latino      M 43.13   NA          41
    ## 22     4     5       ASD     White/Latino      M 47.40    9          NA
    ## 23     4     6       ASD     White/Latino      M 51.37   NA          44
    ## 24     5     1       ASD            White      M 34.03    9          34
    ## 25     5     2       ASD            White      M 37.57   NA          NA
    ## 26     5     3       ASD            White      M 41.57   NA          NA
    ## 27     5     4       ASD            White      M 45.53   NA          38
    ## 28     5     5       ASD            White      M 49.30    9          NA
    ## 29     5     6       ASD            White      M 54.13   NA          40
    ## 30     6     1       ASD      Bangladeshi      F 26.17   17          20
    ## 31     6     2       ASD      Bangladeshi      F 30.27   NA          NA
    ## 32     6     3       ASD      Bangledeshi      F 34.57   NA          NA
    ## 33     6     4       ASD      Bangladeshi      F 38.47   NA          33
    ## 34     6     5       ASD      Bangladeshi      F 42.43    8          NA
    ## 35     6     6       ASD      Bangladeshi      F 46.53   NA          39
    ## 36     7     1       ASD            White      F 41.00   18          24
    ## 37     7     2       ASD            White      F 49.20   NA          NA
    ## 38     7     3       ASD            White      F 49.20   NA          NA
    ## 39     7     4       ASD            White      F 52.27   NA          29
    ## 40     7     5       ASD            White      F 57.73   17          NA
    ## 41     8     1        TD            White      M 20.10    5          32
    ## 42     8     2        TD            White      M 24.07   NA          NA
    ## 43     8     4        TD            White      M 32.07   NA          42
    ## 44     8     5        TD            White      M 36.07    5          NA
    ## 45     8     6        TD            White      M 40.17   NA          43
    ## 46     9     1        TD            White      F 18.30    0          24
    ## 47     9     2        TD            White      F 23.73   NA          NA
    ## 48     9     3        TD            White      F 26.63   NA          NA
    ## 49     9     4        TD            White      F 31.07   NA          45
    ## 50     9     5        TD            White      F 35.00    1          NA
    ## 51    10     1        TD            White      M 19.27    3          27
    ## 52    10     2        TD            White      M 23.83   NA          NA
    ## 53    10     3        TD            White      M 28.53   NA          NA
    ## 54    10     4        TD            White      M 32.43   NA          31
    ## 55    10     5        TD            White      M 36.53    5          NA
    ## 56    10     6        TD            White      M 40.43   NA          33
    ## 57    11     1        TD            White      M 19.23    0          21
    ## 58    11     2        TD            White      M    NA   NA          NA
    ## 59    11     3        TD            White      M 27.27   NA          NA
    ## 60    11     4        TD            White      M 31.20   NA          35
    ## 61    11     5        TD            White      M 35.63    0          NA
    ## 62    11     6        TD            White      M 40.27   NA          42
    ## 63    12     1        TD            White      M 20.07    0          30
    ## 64    12     2        TD            White      M 23.83   NA          NA
    ## 65    12     3        TD            White      M 28.27   NA          NA
    ## 66    12     4        TD            White      M 32.07   NA          42
    ## 67    12     5        TD            White      M 35.87    0          NA
    ## 68    12     6        TD            White      M 41.50   NA          44
    ## 69    13     1        TD            White      M 19.00    0          25
    ## 70    13     2        TD            White      M 22.97   NA          NA
    ## 71    13     3        TD            White      M 27.07   NA          NA
    ## 72    13     4        TD            White      M 31.03   NA          40
    ## 73    13     5        TD            White      M 35.37    0          NA
    ## 74    13     6        TD            White      M 39.40   NA          47
    ## 75    14     1        TD            White      M 18.97    0          23
    ## 76    14     2        TD            White      M 22.87   NA          NA
    ## 77    14     3        TD            White      M 26.80   NA          NA
    ## 78    14     4        TD            White      M 31.47   NA          29
    ## 79    14     5        TD            White      M 35.10    0          NA
    ## 80    14     6        TD            White      M 39.43   NA          44
    ## 81    15     1        TD            White      M 19.27    0          24
    ## 82    15     2        TD            White      M 24.80   NA          NA
    ## 83    15     3        TD            White      M 28.10   NA          NA
    ## 84    15     4        TD            White      M 31.27   NA          33
    ## 85    15     5        TD            White      M 35.90    3          NA
    ## 86    15     6        TD            White      M 40.30   NA          40
    ## 87    16     1        TD            White      M 21.67    0          29
    ## 88    16     2        TD            White      M 26.27   NA          NA
    ## 89    16     3        TD            White      M 30.63   NA          NA
    ## 90    16     4        TD            White      M 34.27   NA          45
    ## 91    16     5        TD            White      M 38.17    0          NA
    ## 92    16     6        TD            White      M 42.93   NA          49
    ## 93    17     1       ASD            White      M 34.80   14          25
    ## 94    17     2       ASD            White      M 38.73   NA          NA
    ## 95    17     3       ASD            White      M    NA   NA          NA
    ## 96    17     5       ASD            White      M 50.93   15          NA
    ## 97    17     6       ASD            White      M 54.43   NA          27
    ## 98    18     1        TD            White      M 20.77    0          29
    ## 99    18     2        TD            White      M 26.13   NA          NA
    ## 100   18     3        TD            White      M 30.03   NA          NA
    ## 101   18     4        TD            White      M 34.43   NA          50
    ## 102   18     5        TD            White      M 38.70    0          NA
    ## 103   18     6        TD            White      M 44.07   NA          50
    ## 104   19     1       ASD            White      M 35.80   11          28
    ## 105   19     2       ASD            White      M 39.93   NA          NA
    ## 106   19     3       ASD            White      M 43.90   NA          NA
    ## 107   19     4       ASD            White      M 47.83   NA          35
    ## 108   19     5       ASD            White      M 51.97    9          NA
    ## 109   19     6       ASD            White      M 56.73   NA          30
    ## 110   20     1       ASD            White      M 18.77    9          26
    ## 111   20     2       ASD            White      M 22.50   NA          NA
    ## 112   20     3       ASD            White      M 26.77   NA          NA
    ## 113   20     4       ASD            White      M 30.80   NA          43
    ## 114   20     5       ASD            White      M 34.13    8          NA
    ## 115   20     6       ASD            White      M 37.30   NA          43
    ## 116   21     1       ASD African American      M 27.53   21          22
    ## 117   21     2       ASD African American      M 31.80   NA          NA
    ## 118   21     3       ASD African American      M 36.17   NA          NA
    ## 119   21     4       ASD African American      M 40.27   NA          27
    ## 120   21     5       ASD African American      M 44.70   19          NA
    ## 121   21     6       ASD African American      M 48.97   NA          26
    ## 122   22     1        TD            White      F 18.93    0          21
    ## 123   22     2        TD            White      F 22.90   NA          NA
    ## 124   22     3        TD            White      F 26.67   NA          NA
    ## 125   22     4        TD            White      F 30.93   NA          40
    ## 126   22     5        TD            White      F 35.13    0          NA
    ## 127   22     6        TD            White      F 39.23   NA          43
    ## 128   23     1       ASD     White/Latino      M 27.37   14          25
    ## 129   23     2       ASD     White/Latino      M 32.10   NA          NA
    ## 130   23     3       ASD     White/Latino      M 35.93   NA          NA
    ## 131   23     4       ASD     White/Latino      M 42.23   NA          29
    ## 132   23     5       ASD     White/Latino      M 44.93   14          NA
    ## 133   23     6       ASD     White/Latino      M 47.50   NA          30
    ## 134   24     1       ASD            White      M 37.47   20          13
    ## 135   24     2       ASD            White      M 42.20   NA          NA
    ## 136   24     3       ASD            White      M 46.37   NA          NA
    ## 137   24     4       ASD            White      M 52.13   NA          29
    ## 138   24     5       ASD            White      M 57.20   17          NA
    ## 139   24     6       ASD            White      M 62.40   NA          30
    ## 140   25     1        TD            White      M 21.77    0          29
    ## 141   25     2        TD            White      M 25.47   NA          NA
    ## 142   25     3        TD            White      M 30.13   NA          NA
    ## 143   25     4        TD            White      M 34.00   NA          47
    ## 144   25     5        TD            White      M 37.93    0          NA
    ## 145   25     6        TD            White      M 42.47   NA          48
    ## 146   26     1       ASD            White      M 30.40   11          32
    ## 147   26     3       ASD            White      M 38.60   NA          NA
    ## 148   26     4       ASD            White      M 42.63   NA          41
    ## 149   26     5       ASD            White      M 46.93    4          NA
    ## 150   26     6       ASD            White      M 51.00   NA          46
    ## 151   27     1        TD            White      M 21.03    0          26
    ## 152   27     2        TD            White      M 25.23   NA          NA
    ## 153   27     3        TD            White      M 29.07   NA          NA
    ## 154   27     4        TD            White      M 33.30   NA          42
    ## 155   27     5        TD            White      M 37.73    0          NA
    ## 156   27     6        TD            White      M 41.17   NA          43
    ## 157   28     1        TD            White      M 19.93    0          20
    ## 158   28     2        TD            White      M 23.57   NA          NA
    ## 159   28     3        TD            White      M 27.03   NA          NA
    ## 160   28     4        TD            White      M 31.00   NA          28
    ## 161   28     5        TD            White      M 36.43    0          NA
    ## 162   28     6        TD            White      M 39.43   NA          39
    ## 163   29     1       ASD            White      M 34.87   17          26
    ## 164   29     2       ASD            White      M 38.13   NA          NA
    ## 165   29     3       ASD            White      M 43.10   NA          NA
    ## 166   29     4       ASD            White      M 46.63   NA          27
    ## 167   29     5       ASD            White      M 50.97   20          NA
    ## 168   29     6       ASD            White      M 55.17   NA          39
    ## 169   30     1       ASD            White      M 36.53   12          31
    ## 170   30     2       ASD            White      M 40.50   NA          NA
    ## 171   30     3       ASD            White      M 44.67   NA          NA
    ## 172   30     4       ASD            White      M 48.10   NA          47
    ## 173   30     5       ASD            White      M 52.90   15          NA
    ## 174   30     6       ASD            White      M 56.43   NA          49
    ## 175   31     1        TD            White      M 23.13    0          27
    ## 176   31     2        TD            White      M 27.70   NA          NA
    ## 177   31     3        TD            White      M 32.07   NA          NA
    ## 178   31     4        TD            White      M 35.03   NA          43
    ## 179   31     5        TD            White      M 39.53    0          NA
    ## 180   31     6        TD            White      M 43.80   NA          45
    ## 181   32     1       ASD            White      M 34.27   21          21
    ## 182   32     2       ASD            White      M 38.33   NA          NA
    ## 183   32     3       ASD            White      M 42.13   NA          NA
    ## 184   32     4       ASD            White      M 47.10   NA          27
    ## 185   32     5       ASD            White      M 50.73   20          NA
    ## 186   32     6       ASD            White      M 54.63   NA          28
    ## 187   33     1       ASD African American      F 25.33   14          25
    ## 188   33     2       ASD African American      F 29.70   NA          NA
    ## 189   33     3       ASD African American      F 34.10   NA          NA
    ## 190   33     4       ASD African American      F 36.93   NA          36
    ## 191   33     5       ASD African American      F 42.97   25          NA
    ## 192   33     6       ASD African American      F 46.17   NA          33
    ## 193   34     1       ASD            White      M 33.77   10          27
    ## 194   34     2       ASD            White      M 38.13   NA          NA
    ## 195   34     3       ASD            White      M 43.00   NA          NA
    ## 196   34     4       ASD            White      M 46.13   NA          31
    ## 197   34     5       ASD            White      M 50.30   16          NA
    ## 198   34     6       ASD            White      M 53.77   NA          44
    ## 199   35     1        TD            White      M 19.30    1          23
    ## 200   35     2        TD            White      M 23.43   NA          NA
    ## 201   35     3        TD            White      M 27.13   NA          NA
    ## 202   35     4        TD            White      M 30.80   NA          36
    ## 203   35     5        TD            White      M 34.93    2          NA
    ## 204   35     6        TD            White      M 39.07   NA          45
    ## 205   36     1        TD            White      M 19.20    3          19
    ## 206   36     2        TD            White      M 22.87   NA          NA
    ## 207   36     3        TD            White      M 26.87   NA          NA
    ## 208   36     4        TD            White      M 30.40   NA          35
    ## 209   36     5        TD            White      M 34.40    2          NA
    ## 210   36     6        TD            White      M 38.53   NA          46
    ## 211   37     1       ASD            White      M 39.50    7          33
    ## 212   37     2       ASD            White      M 43.68   NA          NA
    ## 213   37     3       ASD            White      M 47.73   NA          NA
    ## 214   37     4       ASD            White      M 51.67   NA          49
    ## 215   37     5       ASD            White      M 55.17    4          NA
    ## 216   37     6       ASD            White      M 58.77   NA          50
    ## 217   38     1        TD            White      F 19.87    1          29
    ## 218   38     2        TD            White      F 24.17   NA          NA
    ## 219   38     3        TD            White      F 28.07   NA          NA
    ## 220   38     4        TD            White      F 32.17   NA          44
    ## 221   38     5        TD            White      F 36.13    0          NA
    ## 222   39     1       ASD      White/Asian      M 33.20   11          26
    ## 223   39     2       ASD      White/Asian      M 37.37   NA          NA
    ## 224   39     3       ASD      White/Asian      M 41.73   NA          NA
    ## 225   39     4       ASD      White/Asian      M 45.33   NA          45
    ## 226   39     5       ASD      White/Asian      M 48.63   11          NA
    ## 227   39     6       ASD      White/Asian      M 53.63   NA          30
    ## 228   40     1       ASD         Lebanese      M 24.90   13          27
    ## 229   40     2       ASD         Lebanese      M 32.20   NA          NA
    ## 230   40     3       ASD         Lebanese      M 34.00   NA          NA
    ## 231   40     4       ASD         Lebanese      M    NA   NA          NA
    ## 232   40     6       ASD         Lebanese      M 46.40   NA          41
    ## 233   41     1        TD            White      M 19.23    0          25
    ## 234   41     2        TD            White      M 23.07   NA          NA
    ## 235   41     3        TD            White      M 28.47   NA          NA
    ## 236   41     4        TD            White      M 32.07   NA          41
    ## 237   41     5        TD            White      M 35.67    0          NA
    ## 238   41     6        TD            White      M 39.93   NA          45
    ## 239   42     1        TD            White      M 19.37    0          24
    ## 240   42     2        TD            White      M    NA   NA          NA
    ## 241   42     3        TD            White      M 28.90   NA          NA
    ## 242   42     4        TD            White      M 32.07   NA          39
    ## 243   42     5        TD            White      M 36.40    0          NA
    ## 244   42     6        TD            White      M 40.13   NA          46
    ## 245   43     1       ASD            White      M 37.03   14          42
    ## 246   43     2       ASD            White      M 42.77   NA          NA
    ## 247   43     3       ASD            White      M 46.23   NA          NA
    ## 248   43     4       ASD            White      M 49.30   NA          50
    ## 249   43     5       ASD            White      M 53.40   11          NA
    ## 250   43     6       ASD            White      M 57.37   NA          49
    ## 251   44     1        TD            White      M 19.77    3          30
    ## 252   44     2        TD            White      M 23.63   NA          NA
    ## 253   44     3        TD            White      M 27.67   NA          NA
    ## 254   44     4        TD            White      M 32.07   NA          40
    ## 255   44     5        TD            White      M 35.83    1          NA
    ## 256   45     1        TD            White      M 20.03    5          24
    ## 257   45     2        TD            White      M 24.68   NA          NA
    ## 258   45     3        TD            White      M 28.63   NA          NA
    ## 259   45     4        TD            White      M 32.77   NA          26
    ## 260   45     5        TD            White      M 36.70   10          NA
    ## 261   46     1       ASD            White      M 36.73   20          21
    ## 262   46     2       ASD            White      M 41.50   NA          NA
    ## 263   46     3       ASD            White      M 45.57   NA          NA
    ## 264   46     4       ASD            White      M 50.23   NA          29
    ## 265   46     5       ASD            White      M 54.33   17          NA
    ## 266   46     6       ASD            White      M 57.43   NA          32
    ## 267   47     1        TD            White      F 20.03    0          27
    ## 268   47     2        TD            White      F 24.40   NA          NA
    ## 269   47     3        TD            White      F 29.53   NA          NA
    ## 270   47     4        TD            White      F 32.13   NA          42
    ## 271   47     5        TD            White      F 39.10   NA          NA
    ## 272   47     6        TD            White      F 40.37   NA          43
    ## 273   48     1       ASD            White      M 31.63   17          28
    ## 274   48     2       ASD            White      M 37.20   NA          NA
    ## 275   48     4       ASD            White      M 43.63   NA          33
    ## 276   48     5       ASD            White      M 52.77   17          NA
    ## 277   48     6       ASD            White      M    NA   NA          32
    ## 278   49     1        TD            White      M 23.07    0          27
    ## 279   49     2        TD            White      M 27.47   NA          NA
    ## 280   49     3        TD            White      M 31.63   NA          NA
    ## 281   49     4        TD            White      M 35.63   NA          45
    ## 282   49     5        TD            White      M 39.47    0          NA
    ## 283   49     6        TD            White      M 43.40   NA          45
    ## 284   50     1       ASD            White      M 37.47   19          17
    ## 285   50     2       ASD            White      M 42.20   NA          NA
    ## 286   50     3       ASD            White      M 46.37   NA          NA
    ## 287   50     4       ASD            White      M 52.13   NA          22
    ## 288   50     5       ASD            White      M 57.20   17          NA
    ## 289   50     6       ASD            White      M 62.40   NA          24
    ## 290   51     1        TD            Asian      F 20.87    1          22
    ## 291   51     2        TD            Asian      F 25.40   NA          NA
    ## 292   51     3        TD            Asian      F 29.67   NA          NA
    ## 293   51     4        TD            Asian      F 34.43   NA          30
    ## 294   51     5        TD            Asian      F 37.67    3          NA
    ## 295   51     6        TD            Asian      F 42.10   NA          46
    ## 296   52     1        TD            White      M 22.57    0          29
    ## 297   52     2        TD            White      M 26.63   NA          NA
    ## 298   52     3        TD            White      M 30.77   NA          NA
    ## 299   52     4        TD            White      M 35.03   NA          45
    ## 300   52     5        TD            White      M 38.60    0          NA
    ## 301   52     6        TD            White      M 43.03   NA          47
    ## 302   53     1        TD            White      M 23.90    0          30
    ## 303   53     2        TD            White      M 28.60   NA          NA
    ## 304   53     3        TD            White      M 32.50   NA          NA
    ## 305   53     4        TD            White      M 36.40   NA          45
    ## 306   53     5        TD            White      M 40.07    0          NA
    ## 307   53     6        TD            White      M 44.43   NA          45
    ## 308   54     1        TD            White      M 19.10    1          24
    ## 309   54     2        TD            White      M 23.17   NA          NA
    ## 310   54     3        TD            White      M 27.07   NA          NA
    ## 311   54     4        TD            White      M 30.83   NA          39
    ## 312   54     5        TD            White      M 35.17    0          NA
    ## 313   54     6        TD            White      M 39.30   NA          41
    ## 314   55     1        TD            White      M 19.97    0          26
    ## 315   55     2        TD            White      M 23.93   NA          NA
    ## 316   55     3        TD            White      M 28.03   NA          NA
    ## 317   55     4        TD            White      M 32.03   NA          40
    ## 318   55     6        TD            White      M 41.93   NA          45
    ## 319   56     1       ASD            White      M 35.50   14          27
    ## 320   56     2       ASD            White      M 39.40   NA          NA
    ## 321   56     3       ASD            White      M 44.47   NA          NA
    ## 322   56     6       ASD            White      M    NA   NA          34
    ## 323   57     1       ASD            White      F 41.07   15          28
    ## 324   57     2       ASD            White      F 41.07   NA          NA
    ## 325   57     3       ASD            White      F 49.97   NA          NA
    ## 326   57     4       ASD            White      F 53.90   NA          30
    ## 327   57     5       ASD            White      F 58.27   13          NA
    ## 328   57     6       ASD            White      F 61.70   NA          32
    ## 329   58     1       ASD            White      M 26.00   15          30
    ## 330   58     2       ASD            White      M 30.10   NA          NA
    ## 331   58     3       ASD            White      M 34.27   NA          NA
    ## 332   58     4       ASD            White      M 38.20   NA          40
    ## 333   58     5       ASD            White      M 42.30   16          NA
    ## 334   58     6       ASD            White      M 46.07   NA          45
    ## 335   59     1        TD            White      M 20.80    4          29
    ## 336   59     2        TD            White      M 25.87   NA          NA
    ## 337   59     3        TD            White      M 29.67   NA          NA
    ## 338   59     4        TD            White      M 33.60   NA          34
    ## 339   59     5        TD            White      M 37.34    5          NA
    ## 340   59     6        TD            White      M 41.00   NA          NA
    ## 341   60     1       ASD            White      M 34.00   13          30
    ## 342   60     2       ASD            White      M 38.63   NA          NA
    ## 343   60     3       ASD            White      M 42.47   NA          NA
    ## 344   60     4       ASD            White      M 47.00   NA          47
    ## 345   60     5       ASD            White      M 51.13   15          NA
    ## 346   60     6       ASD            White      M 54.73   NA          50
    ## 347   61     1       ASD            White      M 42.00   15          27
    ## 348   61     2       ASD            White      M 46.77   NA          NA
    ## 349   61     3       ASD            White      M 50.63   NA          NA
    ## 350   61     4       ASD            White      M 54.13   NA          28
    ## 351   61     5       ASD            White      M 58.57   15          NA
    ## 352   61     6       ASD            White      M 62.33   NA          41
    ##     verbalIQ Socialization  MOT_MLU   CHI_MLU types_MOT types_CHI
...
#3
#we are interested in the development of language (number of words) and not just how many words they know on average. If we only keep the average it is impossible to to say anything about different development in ASD- and TD children. 
```
